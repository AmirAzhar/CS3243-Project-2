...
class Sudoku(object):
    ...
    def csp(self, puzzle):
        ...
        # Initialize var_domain, var_constraints
        for r in range(9):
            for c in range(9):
                var_domain[(r, c)] = None
        ...
        for var in var_domain:
            ...
            # puzzle_val == 0 means that the position on the board is a 0
            # Checking the col for non zero, and add into the assigned_val
            # Checking the row for non zero, and add into the assigned_val
            # Assigned_val contains the non zero numbers found along the same col & row
            for c in range(9):
                val = puzzle[row][c]
            ...
            for r in range(subgrid_r, subgrid_r + 3):
                for c in range(subgrid_c, subgrid_c + 3):
                    val = puzzle[r][c]
            ...
            # var_domain for 0 : the possible domain
            # var_domain for non 0 : the puzzle_val itself
            if puzzle_val == 0:
    ...
    # Follow the algorithm provided on L05-Slide38
    ...
    # Follow the algorithm provided on L05-Slide18
